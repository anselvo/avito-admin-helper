...
function sanctionIPSystemAccess() {
    ...
    $(headerBtnContainer).prepend('<button id="ah-system-access-sanction-ip-btn" type="button" class="ah-default-btn ah-sanction-ip-btn" style="" title="">Одобрить IP</button>');

    $('#ah-system-access-sanction-ip-btn').click(function() {
...
function renderSanctionIPPopup(ip, ticketLink) {
    ...
    $('#ah-layer-blackout-popup').append('<div id="sh-sanction-ip-popup" class="ah-default-popup" style="display: none; width: 500px;" ></div>');
    ...
    $(header).append('<span class="ah-popup-title" style="">Одобрение IP <span style="font-weight: 400;">'+ ip +'</span></span><button type="button" class="ah-default-btn ah-btn-small ah-popup-close">x</button>');
    ...
    $(footer).append('<button type="button" class="ah-action-btn" title="" style="" id="sanction-ip-btn">Сохранить правило</button>');
    ...
    $(byTellCallBtn).click(function() {
        if (!isAuthority('ROLE_SANCTION_IP_BY_CALL')) {
    ...
    $('#sanction-ip-btn').click(function() {
        ...
        if ( $(byTellCallBtn).hasClass('ah-active-btn') && isAuthority('ROLE_SANCTION_IP_BY_CALL')) {
