...
function displayUserInfoOnRightPanel(response, assume, currentTicketId) {
    ...
    if (rightPanelSettings.indexOf('rp-id')+1) {
        ...
        // кнопка Портрет только для тестовой группы
        var availUserIdsArr = userCharacteristicsTestGroup.map(function(obj) {
			return obj.id;
		});
        var curUserId = +userGlobalInfo.id;
        var btnVisible = '';
        if (availUserIdsArr.indexOf(curUserId) == -1) {
            btnVisible = 'display: none;';
        }
        
        $('#userInfoTable tbody').append('<tr><td>ID</td><td><a href="/users/user/info/'+id+'" target="_blank">'+id+'</a><span style="" class="sh-messenger-link-wrapper"><a title="Мессенджер" href="/messenger/user/'+id+'" target="_blank" style="" class="sh-messenger-link"></a></span><button type="button" class="sh-default-btn ah-btn-small sh-user-сharacteristics-btn" style="'+ btnVisible +'" id="sh-user-сharacteristics-btn-rp" title="Открыть окно для составления портрета пользователя">Портрет</button></td></tr>');

        $('#sh-user-сharacteristics-btn-rp').click(function() {
...
//---------- индикация агентов из тестовой группы для портрета юзера ----------//
function userCharacteristicsTestGroupNotif() {
    $('#user-characteristics-test-group-notification').remove();

    var allEditables = document.querySelectorAll('.helpdesk-attr-table-td-label');
    try {
        var tagBlock = [].find.call(allEditables, singleItem => singleItem.firstChild.data === 'Исполнитель');
        var tagParent = tagBlock.parentNode;
        var assigneeBlock = tagParent.querySelector('[name="assigneeId"]');
        var assigneeId = +assigneeBlock.value;
    } catch (e) {
        var assigneeId = false;
    }

    if (!assigneeId)
        return;

    // проверка текущего пользователя (не показывать для тестовой группы)
    // var testUserIdsArr = userCharacteristicsTestGroup.map(function(obj) {
    // return obj.id;
    // });
    // var curUserId = +userGlobalInfo.id;
    // if (~testUserIdsArr.indexOf(curUserId)) {
    // return;
    // }

    // проверка ассигни тестового пользователя
    var testUserAdmIdsArr = userCharacteristicsTestGroup.map(function (obj) {
        return obj.adm_user_id;
    });

    if (~testUserAdmIdsArr.indexOf(assigneeId)) {
        var detailsPanel = $('.helpdesk-details-panel');
        var notifBody = '<b>В текущем обращении Исполнителем назначен агент претензионной линии.</b>';
        $(detailsPanel).before('<div id="user-characteristics-test-group-notification" class="sh-helpdesk-notification ah-alert-danger"></div>');
        var notifBlock = $('#user-characteristics-test-group-notification');
        $(notifBlock).html(notifBody);
        $(notifBlock).show();
    }
}
...
function userCharacteristics() {
    // $('#user-characteristics-form').removeOld();

    $('body').append('<div class="ah-no-blocking-popup-fixed" id="user-characteristics-popup" style="height: 486px; overflow: hidden; width: 500px; top: 0; left: 0; right: 0; bottom: 0; margin: auto; padding: 0 10px;"><form style="height: 100%; text-align: left;"></form></div>');
    var popup = $('#user-characteristics-popup');
    var popupForm = $(popup).find('form');

	$(popupForm).append('<div class="ah-popup-header" style=""></div>');
    $(popupForm).append('<div class="ah-popup-body" style="overflow-y: scroll; height: 385px; padding-right: 10px; padding-left: 5px;"></div>');
    $(popupForm).append('<div class="ah-popup-footer" style="text-align: right;"></div>');
	
	var header = $(popup).find('.ah-popup-header');
    var body = $(popup).find('.ah-popup-body');
    var footer = $(popup).find('.ah-popup-footer');
	
	$(header).append('<span class="ah-popup-title">Портрет пользователя </span><span class="sh-user-id"></span><button type="button" class="sh-default-btn ah-btn-small ah-popup-close" id="user-characteristics-close-popup">x</button>');
    ...
    var fields = [{
        ...
        formName: 'theme',
		selectOptions: [
			'Учетная запись и личный кабинет',
			...
			'Безопасность на Avito',
			'Мобильный Avito'
		]
    },{
        name: 'Эмоциональное состояние',
        type: 'select',
        formName: 'emotionalCondition',
		selectOptions: [
			'Дружелюбный и лояльный',
			'Дотошный и придирчивый',
			'Разозлившийся',
			'Важная птица'
		]
    },{
        name: 'Активность и использование VAS',
        type: 'select',
        formName: 'activityAndVas',
		selectOptions: [
			'Очень активный',
			'Активный',
			'Малоактивный'
        ...
        name: 'Злой / Не злой умысел',
        type: 'select',
        formName: 'AngryOrNotReasons',
		selectOptions: [
			'Злой умысел',
			'Не злой умысел'
		]
    },{
        name: 'Другие комментарии',
        type: 'textarea',
        formName: 'otherComments'
    }];

    fields.forEach(function(field) {
		switch (field.type) {
			case 'input':
				$(body).append('<div class="ah-field-group"><div class="ah-field-title">'+ field.name +'</div><input placeholder="'+ field.name +'" type="text" class="ah-form-control" name="'+ field.formName +'" style="width: 100%;"></div>');
				break;
				
			case 'textarea':
				$(body).append('<div class="ah-field-group"><div class="ah-field-title">'+ field.name +'</div><textarea placeholder="'+ field.name +'" type="text" class="ah-form-control" name="'+ field.formName +'" style="min-height: 60px; width: 100%; resize: vertical;"></textarea></div>');
				break;
				
			case 'select':
				var opts = '';
				field.selectOptions.forEach(function(opt) {
					opts += '<option style="color: #000;" value="'+ opt +'">'+ opt +'</option>';
				});
				
				$(body).append('<div class="ah-field-group"><div class="ah-field-title">'+ field.name +'</div><select class="ah-form-control" style="color: #757575;" name="'+ field.formName +'" placeholder="'+ field.name +'"><option value="" style="color: #757575;">Не выбрано</option>'+ opts +'</select></div>');
    ...
    $(footer).append('<button type="button" class="sh-action-btn" id="comment-user-characteristics">Оставить комментарий</button>');

    $('#user-characteristics-close-popup').click(function() {
        $(popup).hide();
    });

    var commentBtn = $('#comment-user-characteristics');
    $(commentBtn).click(function() {
        getCommentDataUserCharacteristics();
    });
	
	var selectElems = $(body).find('select');
	$(selectElems).change(function() {
		if ( !$(this).val() ) {
...
function showUserCharacteristicsPopup() {
    var userId = $('.helpdesk-side-panel a[href*="/users/search?user_id"]:eq(0)').text();
    if (!userId) {
        return;
    }
    ...
    var popup = $('#user-characteristics-popup');
    var popupForm = $(popup).find('form');
    var fields = $(popupForm).find('[name]');
    $(fields).each(function(i, field) {
        $(field).val('');
    });
	
	var selectElems = $(popupForm).find('select');
	$(selectElems).css('color', '#757575');

    var popoupHeader = $(popupForm).find('.ah-popup-header');
    var popupHeaderUserId = $(popoupHeader).find('.sh-user-id');
    $(popupHeaderUserId).html('<a target="_blank" href="https://adm.avito.ru/users/user/info/'+ userId +'">'+ userId +'</a>');

    $(popup).show();
}

function getCommentDataUserCharacteristics() {
    var popup = $('#user-characteristics-popup');
    var popupForm = $(popup).find('form');
    var fields = $(popupForm).find('[name]');
    var comment = '';
    $(fields).each(function(i, field) {
        if (!$(field).val()) return;
        comment +=  $(field).attr('placeholder') +':  '+ $(field).val() + '.\n';
    });
    // console.log(comment);

    var popoupHeader = $(popupForm).find('.ah-popup-header');
    var popupHeaderUserId = $(popoupHeader).find('.sh-user-id');
    var userId = $(popupHeaderUserId).text();

    if (!userId || !comment) {
        alert('Ошибка: не удается оставить комментарий на учетной записи.');
        return;
    }

    // загрузка
    if ($('#sh-loading-layer').length == 0) {
        $('body').append('<div id="sh-loading-layer"><div class="sh-cssload-container"><div class="sh-cssload-whirlpool"></div></div></div>');
    }
    $('#sh-loading-layer').show();
    commentUserCharacteristics(userId, comment);
}

function addUserCharacteristicsBtn() {
    $('#sh-user-сharacteristics-btn').remove();
    // прячем попап
    var popup = $('#user-characteristics-popup');
    $(popup).hide();

    var userIdLink = $('.helpdesk-side-panel a[href*="/users/search?user_id"]:eq(0)');
    var userId = $(userIdLink).text();
    if (!userId) {
        return;
    }

    var userIdTagParent = $(userIdLink).parent();
    ...
    $(userIdTagParent).append('<button type="button" class="sh-default-btn ah-btn-small sh-user-сharacteristics-btn" style="" id="sh-user-сharacteristics-btn" title="Открыть окно для составления портрета пользователя">Портрет</button>');

    $('#sh-user-сharacteristics-btn').click(function() {
        showUserCharacteristicsPopup();
    });
}
//---------- Портрет пользователя ----------//
