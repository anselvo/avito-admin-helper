...
// проверить цессию
function checkCessionUserInfo() {
    let table = $('#dataTable');
    let tableWrapper = table.parents('.table-scroll');
    let rows = table.find('tbody tr');
    let cessionReg = /(цессия|реорганизация)/i;
    let cessionInfo = [];

    rows.each(function (i, item) {
        let row = $(item);
        let commentCell = row.find('td:eq(2)');
        let commentText = commentCell.text();

        if (cessionReg.test(commentText)) {
            let dateCell = row.find('td:eq(0)');
            let dateFormatted = dateCell.text().split(' ')[0].slice(3);
            row.attr('data-cession-value', i);
            cessionInfo.push(`<span class="ah-cession-info-item text-info" data-cession-value="${i}" 
                title="Прокрутить таблицу к комментарию о цессии">${dateFormatted}</span>`);
        }
    });

    tableWrapper.before(`<div class="ah-cession-info-wrapper"></div>`);
    let cessionInfoWrapper = $('.ah-cession-info-wrapper');

    if (cessionInfo.length !== 0) {
        cessionInfoWrapper.append(`<b class="ah-indicators-fired ah-cession-info-title">Проводилась цессия</b> (${cessionInfo.join(' | ')})`);
    } else {
        cessionInfoWrapper.append(`<b class="ah-inactive ah-cession-info-title">Цессия не проводилась</b>`);
    }

    let cessionInfoBlocks = $('.ah-cession-info-item');
    cessionInfoBlocks.click(function () {
        let tableWrapper = table.parents('.table-scroll');
        let targetValue = $(this).data('cessionValue');
        let target = table.find(`tr[data-cession-value="${targetValue}"]`);
        let scrollPosition = tableWrapper.scrollTop() + target.position().top;
        tableWrapper.animate({scrollTop: scrollPosition}, 300);
    });
}