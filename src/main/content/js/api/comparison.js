        ...
        rows.time += `
                <div class="ah-compare-cell" data-entity-id="${item.id}">
                    <div>
                        ...
                        <span data-compare-time="sort" data-entity-id="${item.id}" data-current-time-ms="${parseRuDate(item.time)}" title="Sort Time">${item.time}</span>
                    </div>
                    <div>
                        <span class="ah-compare-items-label">Start time: </span>
                        <span data-compare-time="start" data-entity-id="${item.id}" data-current-time-ms="${parseRuDate(item.startTime)}" title="Start Time">${item.startTime}</span>
                    </div>
                </div>
            `;
...
UsersComparison.prototype.renderEntities = function(parsedEntities) {
    ...
    function renderEntityCells(info) {
        ...
        cellMailChance.insertAdjacentHTML('beforeend',`
            ...
            <a class="ah-compare-users-user-mail ah-visitable-link" data-compare="email" data-compare-formatted="Почта" data-entity-id="${info.id}" target="_blank" 
                href="${global.connectInfo.adm_url}/users/user/info/${info.id}">${info.mail || info.altmail}</a>${info.mail ? '' : ' (ALT)'},
