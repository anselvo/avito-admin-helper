...
function createHyperLinksIpInTechInfo() {
    ...
    if ( $(ipBlock).text().search(regForIp) + 1 ) {
        ...
        $('.sh-ip-links').append(` | <a href="${global.connectInfo.adm_url}/users/search?ip=${text}" target="_blank">users</a>`);
        $('.sh-ip-links').append(` | <a href="${global.connectInfo.adm_url}/items/search?ip=${text}" target="_blank">items</a>`);
        $('.sh-ip-links').append(` | <a href="${global.connectInfo.adm_url}/system/access?ip=${text}" target="_blank">access</a>`);
    }
}

function createHyperLinksInCommentsHeading() {
    if ( $(`div.helpdesk-ticket-single-comment-heading a[href ^= "${global.connectInfo.adm_url}/users/search?email="]`).length > 0 ) return;
    ...
    for (var i = 0; i < $('div.helpdesk-ticket-single-comment-heading').length; i++) {
        if ( $('div.helpdesk-ticket-single-comment-heading').slice(i, i + 1).html().search(regForMail) + 1 ) {
            ...
            $('div.helpdesk-ticket-single-comment-heading').slice(i, i + 1).html( $('div.helpdesk-ticket-single-comment-heading').slice(i, i + 1).html().replace(regForMail, `<a target="_blank" href="${global.connectInfo.adm_url}/users/search?email=$&">$&</a>`) );
        }
    }
}
//++++++++++ Hyperlinks ++++++++++//
...
function getUserBlockTimeFromHistory(userId, timeDisplayNode) {
    ...
    let url = `${global.connectInfo.adm_url}/users/user/info/${userId}/usr_adm_history`;
    ...
    if (localStorage.shAlternateMode == '1') {
        ...
        for (var i = 0; i < n; i++) {
            if ($('.helpdesk-html-view a').slice(i, i + 1).text().search(regAlterSearchMail) + 1) {
                ...
                var newtext = text.replace(regAlterSearchMail, `<a target="_blank" href="${global.connectInfo.adm_url}/items/search?user=$&">$&</a>`);
        ...
        for (var i = 0; i < n; i++) {
            if ($('.helpdesk-html-view a').slice(i, i + 1).text().search(regAlterSearchMail) + 1) {
                var text = $('.helpdesk-html-view a').slice(i, i + 1).text();
                var newtext = text.replace(regAlterSearchMail, `<a target="_blank" href="${global.connectInfo.adm_url}/users/search?email=$&">$&</a>`);
                $('.helpdesk-html-view a').slice(i, i + 1).replaceWith(newtext);
            }
        }
    }
...
function searchUser(email, currentTicketId) {
    ...
    var href = `${global.connectInfo.adm_url}/users/search?email=${email}`;
...
function searchIdInItems(mail, currentTicketId) {
    var hrefAdmShowItems = `${global.connectInfo.adm_url}/items/search?user=${mail}`;
        ...
        if (requestAdmShowItems.readyState === 4 && requestAdmShowItems.status === 200 && localStorage.currentTicketEmail === mail)  {
            ...
            if (rAdmShowItems.indexOf(login[0])+1) {
                if ($(rAdmShowItems).find('.item_user_login:eq(0)').attr('href') == undefined) {
                    ...
                    $('#sh-expected-hacked-userid').attr("href", `${global.connectInfo.adm_url}/users/user/info/${admIdUser}`);
...
function displayUserInfoOnRightPanel(response, assume, currentTicketId) {
    ...
    if (rightPanelSettings.indexOf('rp-item')+1) {
        ...
        $(mainTable).append(`<tr><td>Items</td><td><a href="${global.connectInfo.adm_url}/items/search?user_id=${id}" target="_blank">${items[0]}</a></td></tr>`);
    }
    ...
    if (rightPanelSettings.indexOf('rp-acc')+1) {
        ...
        $(mainTable).append(`<tr><td>Account</td><td><a href="${global.connectInfo.adm_url}/users/account/info/${id}" target="_blank">${wallet}</a> ${checkVasUsageHtml}</td></tr>`);
...
function showParsedItemIdsInTicket(ids) {
    ...
    ids.forEach(function(id) {
        $(parsedContainer).append(`<div class="ah-parsed-item-row"><div class="single-parsed-item-header" data-item-id="${id}"><span class="ah-single-parsed-item"><input class="ah-transparent-checkbox" type="checkbox" id="parsed-item-${id}" value="${id}" checked><label for="parsed-item-${id}"><a target="_blank" href="${global.connectInfo.adm_url}/items/item/info/${id}">${id}</a><span class="parsed-item-info"></span></label></span></div><div class="ah-parsed-item-user-info-details ah-popover" style="left: calc(100% + 10px);"></div></div>`);
    });

    $(footer).append(`<a target="_blank" class="ah-link-btn" data-btn-info="items-search" href="${global.connectInfo.adm_url}/items/search?query=${ids.join('%7C')}" title="Поиск отмеченных объявлений в items/search">Найти</a><button type="button" class="ah-default-btn" style="" data-btn-info="check-users" title="Получение информации об учетных записях, на которых размещены отмеченные объявления">Проверить УЗ</button>`);
    ...
    $(checkboxes).change(function() {
        ...
        $(searchLink).attr('href', `${global.connectInfo.adm_url}/items/search?query=${checkedIds.join('%7C')}`);
...
function getUserInfoByItem(userId) {
    ...
    var url = `${global.connectInfo.adm_url}/users/user/info/${userId}`;
    var xhr = new XMLHttpRequest();
    xhr.open("GET", url, true);
    xhr.send(null);
    xhr.onreadystatechange = function() {
        if (xhr.readyState == 4) {
            ...
            if (isFinite(userId)) {
                $(itemInfo).append(`<span style="margin-left: 4px;">| <b>УЗ</b>: <a target="_blank" href="${global.connectInfo.adm_url}/users/user/info/${userId}">${userId}</a><span class="ah-parsed-item-user-info" style="margin-left: 4px;"><i>инфо</i></span></span>`);
